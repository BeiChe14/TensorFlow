...
# 处理好之后的数据文件
INPUT_DATA = 'C:\Documents\workspace\Resource\\flower_processed_data.npy'

# 保存训练好的模型的路径。这里可以将使用新数据训练得到的完整模型保存下来，
# 如果计算资源充足，还可以在训练完最后的全连接层之后再训练所有的网络层，
# 这样可以使得新模型更加贴近新数据
TRAIN_FILE = 'C:\Documents\workspace\Resource\save_model'
# Google提供的训练好的模型文件地址
CKPT_FILE = 'C:\Documents\workspace\Resource\inception_v3\inception_v3.ckpt'
...
# 不需要从Google训练好的模型中姐在参数。这里就是最后的全连接层，因为在新的问题中
# 要重新训练这一层的参数。这里给出的是参数的前缀
CHECKPOINT_EXCLUDE_SCOPES = 'InceptionV3/Logits, InceptionV3/AuxLogits'
# 需要训练的网络层参数名称，在fine-tuning过程中就是最后的全连接层
...
def main(_):
    ...
    print("%d training examples, %d validation examplesand %d"
          "testing examples." %
          (n_training_example, len(validation_labels), len(training_labels)
    ))
    ...
    # 定义inception-v3模型。因为谷歌给出的只有模型参数取值，所以这里需要在这个代码当中定义
    # inception-v3的模型结构。虽然理论上需要区分训练和测试中使用的模型，也就是说在测试时应
    # 该使用is_training=False,但是因为预先训练好的inception-v3模型中使用的batch normalization
    # 参数与新数据会有差异，导致效果差，所以这里直接使用同一个模型来进行测试。
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        ...
        train_step = tf.train.RMSPropOptimizer(LERANING_RATE).minimize( # RMS均方根
